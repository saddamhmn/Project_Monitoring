...
function bindBuildingUI(){
    ...
    $('btnDeleteBuildingConfirm')?.addEventListener('click',()=>{if(!activeDraft||!activeDraft.id)return;const f=document.createElement('form');f.method='POST';f.action=`/monitoring/${activeDraft.id}`;const c=document.createElement('input');c.type='hidden';c.name='_token';c.value=document.querySelector('meta[name="csrf-token"]').content;const m=document.createElement('input');m.type='hidden';m.name='_method';m.value='DELETE';f.appendChild(c);f.appendChild(m);document.body.appendChild(f);f.submit();});
